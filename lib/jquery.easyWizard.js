...
(function( $ ) {
	...
	var easyWizardMethods = {
		init : function(options) {
			var settings = $.extend( {
				...
				'stepsText' : '{n}. {t}',
				'showButtons' : true,
                                'showBar' : true, // Added to display a common Bootstrap ProgressBar
				'buttonsClass' : '',
				'prevButton' : '< Back',
				'nextButton' : 'Next >',
				'debug' : false,
				...
				if(thisSettings.steps > 1) {
					...
					$steps.each(function(index) {
						...
						$stepsHtml.append('<li'+(!index?' class="current"':'')+' data-step="'+step+'">'+stepText+'</li>');
                                        ...
                                        if(thisSettings.showBar) {
                                                $this.prepend('<div class="wizardBar progress progress-stripped active"><div class="bar" style="width: ' + (1/ thisSettings.steps) *100 + '%"></div></div>');
                                        }
					...
					console.log('Can\'t make a wizard with just one step');
				}
			});
		},
		prevStep : function( ) {
		...
		goToStep : function(step) {
			...
			currentStep = $activeStep.attr('data-step');
                        ...
                        // wizardBar update
                        $('.wizardBar .bar').css('width', (step/ thisSettings.steps) *100 + "%");
	...
	$.fn.easyWizard = function(method) {
		if ( easyWizardMethods[method] ) {
			...
			$.error( 'Method ' +  method + ' doesn\'t exist on jQuery.easyWizard' );
		}
	};
})(jQuery);